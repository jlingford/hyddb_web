...
class HydrogenaseSequence(BaseModel):
    ...
    protein_sequence = models.TextField(
        ...
        help_text=("Protein sequence for the hydrogenase. This will be shown "
                   "in the Browser.")
    )
    ...
    sequence_for_classification = models.TextField(
        ...
        blank=True,
        help_text=("Protein sequence used for classification. This will not "
    ...
    oxygen_tolerance_predicted = models.CharField(
        ...
        max_length=25,
        choices=OXYGEN_TOLERANCE
    )

    subunits_predicted = models.CharField(
        "Subunits (Predicted)",
        max_length=256
    )
...
class GeneticOrganisation(BaseModel):
    ...
    hydrogenase_class = models.ForeignKey(HydrogenaseClass)
...
class Structure(BaseModel):
    ...
    def pdb_url(self):
        return 'http://www.rcsb.org/pdb/explore/explore.do?pdbId={}'.format(
            self.pdb_id
        )
