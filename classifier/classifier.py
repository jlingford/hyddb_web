...
class BLASTWrapper(object):
    ...
    def __init__(self, db=None, check_version=True):
        ...
        if check_version and _check_version() < required_version:
            raise Exception('Required BLAST+ version is {}'.
    ...
    def search(self, seqs, max_target_seqs):
        ...
        cmd = ' '.join(['blastp',
                        ...
                        '-db', os.path.basename(self.db),
                        '-outfmt', '"6 qseqid stitle bitscore evalue"'])
        ...
        return pd.read_csv(StringIO(process.stdout), sep='\t',
...
class BLASTClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, no_neighbors=1, **kwargs):
        ...
        if no_neighbors < 1:
            raise ValueError('Argument `no_neighbors` must be larger'
    ...
    def predict_full(self, X):
        ...
        results = self._blast.search(seqcol,
                                     max_target_seqs=self.no_neighbors)
